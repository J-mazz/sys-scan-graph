...
static CanonVal build_canonical(const Report& report, long long total_risk_all, long long emitted_risk, size_t finding_total_all, size_t scanners_with_findings, long long duration_ms, const std::string& slowest_name, long long slowest_ms, std::chrono::system_clock::time_point earliest, std::chrono::system_clock::time_point latest, const std::map<std::string,size_t>& severity_counts_all, const std::map<std::string,size_t>& severity_counts_emitted, const HostMeta& host){ bool zero_time=!!std::getenv("SYS_SCAN_CANON_TIME_ZERO"); if(zero_time){ earliest={}; latest={}; duration_ms=0; } CanonVal root{CanonVal::T_OBJ}; auto put_str=[&](CanonVal& o,const std::string& k,const std::string& v){ o.obj[k].type=CanonVal::T_STR; o.obj[k].str=v; }; auto put_num=[&](CanonVal& o,const std::string& k,long long v){ o.obj[k].type=CanonVal::T_NUM; o.obj[k].str=std::to_string(v); }; CanonVal meta{CanonVal::T_OBJ}; put_str(meta,"$schema","https://github.com/J-mazz/sys-scan/schema/v2.json"); put_str(meta,"arch",host.arch); if(!config().no_cmdline_meta && !host.cmdline.empty()) put_str(meta,"cmdline",host.cmdline); if(!config().no_user_meta){ put_str(meta,"egid",std::to_string(host.egid)); put_str(meta,"euid",std::to_string(host.euid)); put_str(meta,"gid",std::to_string(host.gid)); put_str(meta,"uid",std::to_string(host.uid)); put_str(meta,"user",host.user);} if(!config().no_hostname_meta) put_str(meta,"hostname",host.hostname); put_str(meta,"json_schema_version","2"); put_str(meta,"kernel",host.kernel); put_str(meta,"os_id",host.os_id); if(!host.os_pretty.empty()) put_str(meta,"os_pretty",host.os_pretty); if(!host.os_version.empty()) put_str(meta,"os_version",host.os_version); put_str(meta,"tool_version", buildinfo::APP_VERSION); if(std::getenv("SYS_SCAN_CANON_TIME_ZERO")) put_str(meta,"normalized_time","true");
    // Post-construction hardening (idempotent): enforce suppression flags even if earlier logic added fields
    if(config().no_user_meta){ meta.obj.erase("uid"); meta.obj.erase("euid"); meta.obj.erase("gid"); meta.obj.erase("egid"); meta.obj.erase("user"); }
    if(config().no_cmdline_meta){ meta.obj.erase("cmdline"); }
    if(config().no_hostname_meta){ meta.obj.erase("hostname"); }
