...
public:
    ...
    void scan(Report& report) override {
        ...
        if(!skel){ report.add_error(name(), "open failed"); return; }
        if(process_exec_bpf__load(skel)) { report.add_error(name(), "load failed"); process_exec_bpf__destroy(skel); return; }
        if(process_exec_bpf__attach(skel)) { report.add_error(name(), "attach failed"); process_exec_bpf__destroy(skel); return; }
    // Get ring buffer file descriptor from the events map
    int ring_fd = bpf_map__fd(skel->maps.events);
    if(ring_fd < 0){ report.add_error(name(), "failed to get ring buffer fd"); process_exec_bpf__destroy(skel); return; }
        ...
        if(!rb){ report.add_error(name(), "ring buffer create failed"); process_exec_bpf__destroy(skel); return; }
