...
void IOCScanner::scan(Report& report) {
    ...
    for(auto& kv : proc_hits){
        ...
        if(a.any_pattern) f.metadata["pattern_match"] = "true"; if(a.any_deleted) f.metadata["deleted_exe"] = "true"; if(a.any_ww_exec) f.metadata["world_writable_exec"] = "true"; if(!a.exe.empty()) f.metadata["exe"] = a.exe; f.metadata["pid_count"] = std::to_string(a.pids.size());
    ...
    for(auto& root : temp_roots){
        std::error_code ec;
        for(auto it = fs::recursive_directory_iterator(root, fs::directory_options::skip_permission_denied, ec); it!=fs::recursive_directory_iterator(); ++it){ if(ec) break; const auto& p = it->path(); if(!it->is_regular_file(ec)) continue; if(is_executable(p)){ Finding f; f.id = p.string(); f.title = "Executable in temp"; f.severity="high"; f.description="Executable file located in temporary directory"; report.add_finding(this->name(), std::move(f)); } }
    }
    // Heuristic 3: Unexpected SUID in home directories
    for(const auto& entry : fs::directory_iterator("/home")){
        if(!entry.is_directory()) continue; std::error_code ec; for(auto it = fs::recursive_directory_iterator(entry.path(), fs::directory_options::skip_permission_denied, ec); it!=fs::recursive_directory_iterator(); ++it){ if(ec) break; if(!it->is_regular_file(ec)) continue; if(has_suid(it->path())) { Finding f; f.id = it->path().string(); f.title = "SUID in home"; f.severity="critical"; f.description="SUID binary present in user home"; report.add_finding(this->name(), std::move(f)); } }
    }
    ...
    if(fs::exists("/etc/ld.so.preload")){
        std::ifstream pfs("/etc/ld.so.preload"); std::string line; while(std::getline(pfs,line)){ if(line.empty()) continue; std::string lib = line; struct stat st{}; if(stat(lib.c_str(), &st)==0){
                bool ww = (st.st_mode & S_IWOTH); if(ww){ Finding f; f.id = lib; f.title="World-writable preload library"; f.severity="critical"; f.description="Library in ld.so.preload is world-writable"; report.add_finding(this->name(), std::move(f)); }
            } else {
                Finding f; f.id = lib; f.title="Missing preload library"; f.severity="medium"; f.description="Entry in ld.so.preload missing on disk"; report.add_finding(this->name(), std::move(f)); }
        }
    }
}
