...
int main(){
    ...
    for(const auto& sr: res){ if(sr.scanner_name=="suid_sgid"){ for(const auto& f: sr.findings){ if(f.id.find("/passwd")!=std::string::npos || f.id.find("/sudo")!=std::string::npos){ saw=true; if(f.severity=="low") downgraded=true; } } } }
